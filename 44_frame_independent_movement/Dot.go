...
//Move moves the dot
func (d *Dot) Move(timeStep float64) {
	...
	if d.mPosX < 0 {
		d.mPosX = 0
	} else if d.mPosX > screenWitdh-DotWidth {
		d.mPosX = screenWitdh - DotWidth
	}
	...
	if d.mPosY < 0 {
		d.mPosY = 0
	} else if d.mPosY > screenHeight-DotHeight {
		d.mPosY = screenHeight - DotHeight
	}
}
...
func (d *Dot) Render() error {
	//Show the dot
	err := gDotTexture.Render(int32(d.mPosX), int32(d.mPosY), nil, 0, nil, sdl.FLIP_NONE)
	if err != nil {
